document.addEventListener('DOMContentLoaded', () => {
  ...
  const addHighlightTool = () => {
    ...
    const { highlightCopy, highlightLang, highlightHeightLimit, highlightFullpage, highlightMacStyle, plugin, highlightWordWrap } = highLight
    const isHighlightShrink = GLOBAL_CONFIG_SITE.isHighlightShrink
    const isShowTool = highlightWordWrap || highlightCopy || highlightLang || isHighlightShrink !== undefined || highlightFullpage || highlightMacStyle
    ...
    const highlightWordWrapEle = highlightWordWrap ? '<i class="fas fa-minus word-wrap-button"></i>' : ''
    ...
    const toggleWordWrap = (codeElement, target) => {
      if (codeElement.style.whiteSpace === 'pre-wrap') {
        codeElement.style.whiteSpace = 'inherit'
        target.remove('fa-plus')
        target.add('fa-minus')
      } else {
        codeElement.style.whiteSpace = 'pre-wrap'
        target.remove('fa-minus')
        target.add('fa-plus')
      }
    }
    ...
    const highlightToolsFn = e => {
      ...
      else if ($target.contains('word-wrap-button')) {
        const codeElement = currentElement.parentElement.querySelector('code')
        if (codeElement) {
          toggleWordWrap(codeElement, $target)
        }
      }
    }
    ...
    const createEle = (lang, item) => {
      ...
      if (isShowTool) {
        ...
        hlTools.innerHTML = highlightMacStyleEle + highlightShrinkEle + lang + highlightCopyEle + highlightFullpageEle + highlightWordWrapEle
